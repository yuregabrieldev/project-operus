...
server.tool("get_figma_components", {
    file_key: z.string().describe("The Figma file key"),
}, async ({ file_key }) => {
    try {
...
server.tool("get_figma_images", {
    ...
    try {
        const ids = node_ids.join(",");
        const response = await figmaApi.get(`/images/${file_key}?ids=${ids}&format=${format}&scale=${scale}`);
        return {
            content: [
                {
                    type: "text",
