    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {productGroups.map((group) => (
                <Card key={group.product.id} className="overflow-hidden border-l-4 transition-shadow hover:shadow-lg" style={{ borderLeftColor: group.stockStatus === 'critical' ? '#ef4444' : group.stockStatus === 'low' ? '#f59e0b' : '#10b981' }}>
                    <CardContent className="p-4">
                        {/* Header: Product Name + Total */}
                        <div className="flex items-start justify-between mb-3">
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                                {/* Product Image or Placeholder */}
                                <div className="w-14 h-14 rounded-lg bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center flex-shrink-0 overflow-hidden border border-slate-200">
                                ...
                                <div className="min-w-0">
                                    <h3 className="font-semibold text-sm text-gray-900 truncate">{group.product.name}</h3>
